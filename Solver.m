...
function [ycell,action]=Solver(par,signal,SS)
...
params.Dimension = 2; %The dimension of the system.
params.c = 1e18; %The remeshing parameter, c. Larger values give greater remeshing.
...
params.Remesh = 1; %Turn the remesh on or off. If off, the algorithm will be defauMESClt perform K*MaxIter itereations.
params.q = 10; %The q parameter from the original paconstraints can be set of the path as well. This is not done in our per. Is a measure of path smoothness.
...
N=2;  %%the dimension od the system
% xx=zeros(cycle_index,N);

% %%Solve odes from different initial values
% for i=1:cycle_index
%     x0=5*rand(1,N);
%     [t,x]=ode45(@(t,x)MISA(t,x,par,signal),[0,100],x0);
%     newx=x(end,:);
%     x=inf*ones(1,N);
%     tot_time = 100;
%     while (norm( x(end,:)-newx(end,:) ,2 )>1e-7 && tot_time < 2000)
%         x=newx;
%         [t,newx]=ode45(@(t,x)MISA(t,x,par,signal),[0,100],x(end,:));
...
Func=@(x)MISA(1,x,par,signal);  %Force
dFunc=[]; %Jacobian
feasi_n=2; % number of steady states
